...
// Check that all symbols (variable & functions) wrapped in sciCV 5.0.0 (OpenCV 5.0.0) are still mapped

scicv_Init();

// Functions
exec(fullfile(get_scicv_path(), "tests", "unit_tests", "functions-5.0.0.txt"), -1);
...
// Variables
exec(fullfile(get_scicv_path(), "tests", "unit_tests", "variables-5.0.0.txt"), -1);
